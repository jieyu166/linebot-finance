...
| 第一銀行 | ✅ | | |
| 永豐銀行 | ✅ | ✅ | ✅（國內+復委託，獨立交割戶）|
| 玉山銀行 | ✅ | ✅ | 直接從銀行帳戶交割（無獨立證券帳戶）|
| 台新銀行 | ✅ | ✅（Richart）| ✅（直接從銀行帳戶交割，無獨立證券帳戶）|
| 富邦銀行 | ✅ | ✅ | |
| 國泰銀行 | ✅ | | |
| LINE Bank | | ✅ | |
| 王道銀行 | | ✅ | |
...
| 永豐外幣 | 永豐銀行 | USD | 銀行 | |
| 玉山 | 玉山銀行 | TWD | 銀行 | 可直接買股（交割戶即此帳戶） |
| 玉山信用卡 | 玉山銀行 | TWD | 信用卡 | 玉山 |
| 台新 | 台新銀行 | TWD | 銀行 | Richart；可直接買股（交割戶即此帳戶） |
...
- **證券交割兩種模式**：永豐證券有獨立交割戶（帳戶類型=證券，如「永豐證券」），證券對帳單交易一律記到該證券帳戶；玉山、台新沒有獨立證券帳戶，買賣股票直接從既有的銀行帳戶（類型=銀行，如「玉山」「台新」）扣款／入帳，證券對帳單的 `account` 會 fallback 對應到同銀行的銀行帳戶。兩種模式下，證券對帳單都是股票名稱（如「台積電」）的權威來源；銀行明細上概括的「證券交割」「股款」列會在匯入時交由去重機制（見下）以具體股名回填覆蓋，避免重複記帳。
- **去重**：`dedupeAgainstSheet()` 比對同帳戶、同金額、同類型、日期 ±2 天內的既有匯入列（僅比對 PDF匯入／文字匯入／自動配對來源），避免證券對帳單與銀行明細重複記帳；證券對帳單的具體股名會回填覆蓋既有的概括品項（如「定期買股」「交割」「證券」「股票」「股款」「申購」等 `GENERIC_STOCK_ITEMS` 樣式）。
