...
function buildSystemPrompt(expenseCategories, incomeCategories) {
  return '你是一個記帳助手。使用者會傳送記帳訊息，你必須解析並回傳 JSON。\n\n'
    ...
    + '## 範例\n'
    + '輸入：午餐80\n輸出：{"type":"支出","category":"飲食","item":"午餐","description":"午餐80","institution":"現金","account":"","currency":"TWD","amount":80}\n\n'
    + '輸入：玉山信用卡 加油1500\n輸出：{"type":"支出","category":"交通","item":"加油","description":"玉山信用卡加油","institution":"玉山銀行","account":"信用卡","currency":"TWD","amount":1500}\n\n'
    + '輸入：飲食 午餐便當 80\n輸出：{"type":"支出","category":"飲食","item":"午餐便當","description":"午餐便當","institution":"現金","account":"","currency":"TWD","amount":80}\n\n'
    + '輸入：收到薪水50000\n輸出：{"type":"收入","category":"薪資","item":"薪水","description":"收到薪水","institution":"現金","account":"","currency":"TWD","amount":50000}\n\n'
    + '輸入：給爸媽生活費10000\n輸出：{"type":"支出","category":"父母","item":"生活費","description":"給爸媽生活費","institution":"現金","account":"","currency":"TWD","amount":10000}\n\n'
    + '輸入：日本旅遊買藥妝3000日幣\n輸出：{"type":"支出","category":"購物","item":"藥妝","description":"日本旅遊買藥妝","institution":"現金","account":"","currency":"JPY","amount":3000}\n\n'
    + '只回傳 JSON，不要有任何其他文字。';
}
 ...
 */
function parseWithOpenAI(message, expenseCategories, incomeCategories) {
  var apiKey = getConfig('OPENAI_API_KEY');
  var url = 'https://api.openai.com/v1/chat/completions';

  var systemPrompt = buildSystemPrompt(expenseCategories, incomeCategories);
 ...
 * @returns {string} system prompt
 */
function buildPdfSystemPrompt(expenseCategories, incomeCategories) {
  return '你是一個台灣銀行帳單解析助手。使用者會提供從銀行帳單 PDF 擷取的文字內容，你必須批次解析所有交易並回傳 JSON。\n\n'
    ...
    + '## 必須跳過的行（不計入交易）\n'
    + '1. 繳款/自動扣繳行：含「自動扣繳」「自扣已入帳」「繳款」「感謝您」且金額為負數。\n'
    ...
    + '6. 貸款相關：貸款利息、償還本金、放款繳款 → type:支出, category:貸款。\n'
    + '7. 轉帳相關：跨行轉帳、網路轉帳、轉帳支取 → type:支出, category:轉帳。\n'
    + '   轉帳存入、跨行轉入、CD轉收 → type:收入, category:轉帳（收入分類中歸「其他」）。\n'
    + '8. 信用卡款扣繳（從銀行帳戶扣信用卡費）→ type:支出, category:轉帳。\n'
    + '9. 發票獎金：→ type:收入, category:獎金。\n'
    + '10. 「連加*」前綴為感應支付消費，去除前綴後保留商店名稱。\n'
    + '11. LINE Bank 注意：主帳戶的「刷卡交易」和簽帳金融卡明細是同一筆，只記一次（記簽帳金融卡明細）。\n\n'
    ...
    + '## 輸出格式（嚴格 JSON）\n'
    + '{"bank":"銀行名稱","transactions":[{"date":"yyyy/MM/dd","type":"支出或收入","category":"分類名稱","item":"品項","description":"明細描述","institution":"金融機構","account":"帳戶名稱(如:信用卡末四碼/活期帳戶)","currency":"幣別","amount":金額正數}],"skipped":跳過的行數}\n\n'
    ...
    + '輸出：{"bank":"第一銀行","transactions":[{"date":"2026/03/07","type":"支出","category":"購物","item":"南紡購物中心","description":"連加*南紡購物中心","institution":"第一銀行","account":"信用卡(7142)","currency":"TWD","amount":1901},{"date":"2026/03/13","type":"支出","category":"手續費","item":"國外交易手續費","description":"國外交易手續費(670.00 TWD)","institution":"第一銀行","account":"信用卡(7142)","currency":"TWD","amount":10},{"date":"2026/04/02","type":"收入","category":"回饋","item":"現金回饋","description":"現金回饋-iLEO信用卡","institution":"第一銀行","account":"信用卡(7142)","currency":"TWD","amount":58}],"skipped":2}\n\n'
    ...
    + '輸出：{"bank":"永豐銀行","transactions":[{"date":"2026/03/02","type":"收入","category":"回饋","item":"大戶回饋","description":"大戶回饋","institution":"永豐銀行","account":"活期帳戶","currency":"TWD","amount":607},{"date":"2026/03/09","type":"支出","category":"轉帳","item":"永豐卡費","description":"永豐卡費扣繳","institution":"永豐銀行","account":"活期帳戶","currency":"TWD","amount":21207},{"date":"2026/03/21","type":"收入","category":"利息","item":"利息存入","description":"利息存入","institution":"永豐銀行","account":"活期帳戶","currency":"TWD","amount":356}],"skipped":0}\n\n'

    + '### 範例3：永豐證券\n'
    + '輸入：2026/03/03 普賣 國產 1,000 39.5500 39,550 56 118 39,376\n'
    + '2026/03/06 普買 台積電 5 1,892.0000 9,460 1 9,461\n'
    + '輸出：{"bank":"永豐證券","transactions":[{"date":"2026/03/03","type":"收入","category":"投資獲利","item":"國產","description":"普賣 國產 1,000股","institution":"永豐證券","account":"證券帳戶","currency":"TWD","amount":39376},{"date":"2026/03/06","type":"支出","category":"投資","item":"台積電","description":"普買 台積電 5股","institution":"永豐證券","account":"證券帳戶","currency":"TWD","amount":9461}],"skipped":0}\n\n'
 ...
 */
function parsePdfWithOpenAI(text, expenseCategories, incomeCategories) {
  var apiKey = getConfig('OPENAI_API_KEY');
  var url = 'https://api.openai.com/v1/chat/completions';

  var systemPrompt = buildPdfSystemPrompt(expenseCategories, incomeCategories);
