...
| 永豐信用卡外幣 | 永豐銀行 | USD | 信用卡 | 永豐大戶 | 雙幣卡美元帳單（幣倍卡外幣消費） |
...
- `initializeSheets()` 的預設帳戶改為上表 21 個；既有工作表不覆蓋，改提供 `upsertDefaultAccounts()` 只補缺少的帳戶列與 H、I 欄。
...
### 依真實帳單修正的匯入規則（永豐 2026/08 三份對帳單交叉比對）

1. **信用卡帳單的「回饋入帳戶」列一律跳過**（大戶消費回饋入帳戶_任務/國內/通路、幣倍卡國外消費回饋金入帳戶）：這些回饋實際入銀行帳戶，銀行明細的「大戶回饋」「幣倍回饋」已記為收入。現行 prompt 規則「從描述提取 207」會重複，移除。
2. **雙幣信用卡**：臺幣區塊記到 TWD 卡帳戶，美元區塊記到「永豐信用卡外幣」(USD)。prompt 要求每筆輸出 `currency`，後端依卡帳戶幣別分流。
3. **交割戶與證券對帳單重複**：交割戶銀行明細的「定期買股」「交割」「證券買賣」列跳過，以證券對帳單為主（有股名、股數）。匯入時以「同帳戶、同金額、日期 ±2 天、來源為匯入」判定重複：證券對帳單後到時改寫既有列的品項／明細，不新增；銀行明細後到時跳過。回覆訊息顯示「跳過 N 筆與既有紀錄重複」。
4. **外幣卡費**：銀行 TWD 帳戶的「卡費換匯」列記支出／繳信用卡，配對到 USD 卡帳戶的「卡費入帳」，USD 金額取自同帳單外幣過渡帳號（198-00）的「永豐卡費」＋「換匯卡費」合計；過渡帳號其餘列跳過，但「轉帳 ⋯ 幣倍回饋」記到 USD 卡帳戶為收入／回饋。
5. **信用卡帳單日期**：只有 MM/DD，年份由結帳日推算（消費月份大於結帳月份則為前一年）。
6. **真實重複交易不去重**：同日同商店同金額兩筆（如 XSOLLA 670×2）是兩筆交易；只去除 PDF 分頁造成整段重複的情況（比對連續多筆完全相同的區塊）。
7. **餘額驗收**：以帳單月底餘額當驗收值。永豐大戶 0、永豐證券 0、永豐外幣 USD 1,858.62、永豐信用卡 −0、永豐信用卡外幣 −0（初始餘額設 7 月底／上期應繳）。

### 依玉山／台新／中信／一銀帳單補充的規則（見 test/fixtures/other-banks.expected.md、firstbank-card-2026-07.expected.md）

1. **J 欄「帳號識別」允許逗號分隔多個**：同一銀行多個子帳號都屬同一帳戶（玉山 `0015977,0381979`、台新 `288810,288815,288818`、一銀 `630`）。比對時去掉 `-` 與前導零後做前綴比對。
2. **同帳戶內轉跳過**：交易所在帳號與對方帳號都對應到同一帳戶名稱（玉山 481 ↔ 221 的 24,000）→ 兩列皆跳過。
3. **對方帳號比對取代單靠銀行代碼**：取描述／對方帳號欄最長數字串 d，候選 = [d, d 去前 3 碼]，去前導零後與各帳戶 J 欄識別前綴比對，唯一命中即為對方帳戶（中信 `0019801800104436` → 永豐大戶；`0000381979084481` → 玉山；`0000063057052425` → 一銀）。找不到再用銀行代碼對照表推機構。
4. **信用卡帳單回饋兩種型態**：描述含「入帳戶」→ 跳過（永豐，回饋入銀行）；否則金額為負且含「回饋」→ 收入／回饋記在卡帳戶（玉山 UBear、一銀 iLEO，直接折抵卡費）。
5. **ATM 提款 → 現金**：摘要含「現金提」「ATM提款」「提款」的支出 → 支出／轉帳，自動建立「現金」帳戶的收入／轉帳並配對。
6. **PDF 亂碼改逐行過濾**：一銀卡帳單含大量條碼亂碼，現行「亂碼比例 > 30% 整份拒收」改為丟掉亂碼行（非中英數標點字元比例 > 50% 的行），整份仍可解析。
7. **tab 分隔文字**（中信網頁複製）：支出／存入欄位明確，prompt 說明 tab 分隔時直接依欄位取方向。
8. **一張帳單多張卡**（一銀 iLEO＋綠活、永豐三卡）：同一卡帳戶，卡號末四碼寫進 description。
9. **外幣帳戶餘額趨近 0 且無交易**（玉山 JPY/USD、台新外幣）：跳過，不需建帳戶。
...
- Prompt 測試：`test/fixtures/`（已加入 .gitignore，不上傳）存放使用者提供的真實帳單與對應的 `*.expected.md` 預期結果。目前有永豐銀行、永豐信用卡、永豐金證券 2026/08 三份，同月可交叉驗證配對與去重。測試腳本以 fixture 呼叫 OpenAI 並比對預期交易清單（帳戶、類型、分類、金額）與配對結果。
